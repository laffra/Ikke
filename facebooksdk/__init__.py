...
class GraphAPI(object):
    ...
    def delete_permissions(self, user_id):
        """Deletes the permissions object from the graph."""
        self.request("{0}/{1}/permissions".format(self.version, user_id), {}, method="DELETE")
    ...
    def request(
        ...
        if result and isinstance(result, dict) and result.get("error"):
            raise GraphAPIError('%s: %s' % (result['error']['message'], path))
        return result
